...
func add_rooms(num_left) -> int:
	...
	for i in range(0,num_openings):
		...
		match open_rooms[i]:
			...
			"south":
				change = await add_south_room(num_remaining)
			"west":
...
func add_east_room(num_left) -> int:
	...
	if num_left > 0:
		...
		overlapping_room = await room.overlapping()
		change = -1
	print("%s overlap with %s" % [self, overlapping_room])
	if num_left <= 0 or overlapping_room != null:
		if room != null:
			await room.queue_free()
...
func add_south_room(num_left) -> int:
	var room = null
	var overlapping_room = null
	var change = 0
	if num_left > 0:
		north_entrance_rooms.shuffle()
		room = load(north_entrance_rooms[0]).instantiate()
		room.set_north_position($South.position)
		room.entrance = entrance
		room.par = self
		add_child(room)
		overlapping_room = await room.overlapping()
	...
	if num_left <= 0 or overlapping_room != null:
		if room != null:
			await room.queue_free()
		room = load(south_cap).instantiate()
		room.set_north_position($South.position)
		room.entrance = entrance
		room.par = self
		add_child(room)
...
func set_gen_options():
	_ready()
	north_entrance_rooms = [room1, room2]
